...
在开始接收并处理请求之前，管理员需要启动一个或者多个 HTTP 服务器。服务器可以使用`HTTPServer`命名空间中的函数进行配置。启动服务器时，您可以选择等服务器异常终止（通常不会发生这种事情除非进程终止），或者从每一个服务器接收`LauchContext`（启动环境）对象，该对象允许终止和等待的操作。

每个 Perfect HTTP 服务器都包括一个名称和一个监听端口，一个以上的路由处理器，以及可以选择的请求过滤器和响应过滤器（一个或多个，或者没有也行）。此外，加密的HTTPS服务器还需要配置TLS信息，比如证书或密钥的文件路径等等。

您可以根据需要任选以下三种方式之一进行启动HTTP服务器。

1. 使用配置文件（配置文件路径、配置文件对象变量或者一个配置字典变量）；
2. 使用 HTTPServer.launch 启动函数；
3. 手工初始化 HTTPServer 对象，设置其属性后调用`start()`函数。
