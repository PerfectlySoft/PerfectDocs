...
下一步是创建一个用户，用于运行所有命令并启动最终The next step is creating a user to run all commands through and also run the final app since it's not that nice to do all as root. Since we're still connected as root we're going to skip the `sudo` commands since there's no point in using it. Please adjust the `USERNAME` and `USER_PASSWORD` variables in the command below.
```
